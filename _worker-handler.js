...
async function requestBody(req) {
  ...
  // Vercel may parse JSON requests before invoking the function. In that
  // case the request stream has already been consumed, so serialise the
  // parsed body instead of attempting to read it a second time.
  if (req.body != null) {
...
export async function handleWorkerRequest(req, res, pathname) {
  try {
    ...
    // The body may be re-serialised after Vercel's JSON parser has run, so
    // the original transport headers are no longer reliable.
